...
## Frontend Track (Phases 0-3)

The frontend follows the backend contract and should feel polished before ranking exists. Phases 0 and 1 are intentionally UI-heavy so the demo reads like a product, not a placeholder.

### Frontend Phase 0 — Shell, Theme, and Case Dashboard

**Goal:** Build a strong app shell and landing experience.

**UI focus:**

- Use a deliberate visual system with strong typography, card hierarchy, and status colors
- Replace flat placeholder screens with a dashboard that has a hero, summary metrics, and recent cases
- Make the global shell responsive with sticky navigation, active states, and polished loading and error states
- Keep the workspace visually consistent across home, cases, and case detail views

**Deliverables:**

- Root layout and shared navigation
- Landing dashboard with CTA, recent cases, and summary cards
- Reusable status badges, card styles, and layout primitives
- Typed API client and shared TypeScript contracts

**Gate:** The app starts, `/` and `/cases` render cleanly, and the shell already feels demo-ready.

### Frontend Phase 1 — Case Workspace and Requirements UI

**Goal:** Make case browsing, requirements, and supplier summaries readable at a glance.

**UI focus:**

- Turn the case page into a structured workspace with a strong header, status chip, and sticky tabs
- Present requirements in grouped sections for mandatory and preference logic
- Show supplier summaries as cards or rows with status, country, reference, and quick actions
- Add subtle hierarchy so the user can immediately tell what matters most

**Deliverables:**

- Cases list with polished cards and clear status treatment
- Case workspace layout with tabs and header summary
- Case overview with supplier summary cards and quick action buttons
- Requirements screen with mandatory/preference grouping, weights, operators, and units

**Gate:** A user can open a case, understand its requirements quickly, and inspect supplier summaries without touching raw JSON.

### Frontend Phase 2 — Documents and Evidence Traceability

**Goal:** Expose the document and evidence trail in the case UI.

**Frontend work:**

- Add document status surfaces for upload, parsing, extraction, ready, and error states
- Add evidence panels or drawers linked from supplier and requirement views
- Show raw value, normalized value, confidence, quote, and source location together
- Make conflicting or low-confidence evidence visually distinct

**Best route fit in the current app:**

- Extend the case overview page to show supplier document and evidence summaries
- Reuse the case workspace shell and tabs instead of adding noisy new pages

**Gate:** Document and evidence state is visible in the UI without reading raw API responses, and traceability feels natural from supplier to evidence to source quote.

### Frontend Phase 3 — Real Eligibility Matrix

**Goal:** Render the backend eligibility result clearly and enforce the PASS-only boundary in the UI.

**Frontend work:**

- Connect the eligibility page to the real backend response
- Render the supplier-by-requirement matrix with PASS, FAIL, REVIEW, and PENDING states
- Add overall supplier status, row emphasis, and cell-level details
- Provide an evidence drawer for the selected check with the reason code and citation data

**UI focus:**

- PASS suppliers should stand out as the only ones eligible for ranking later
- FAIL, REVIEW, and PENDING should be visually excluded, not shown as zero-score winners
- The matrix should read like a decision surface, not a spreadsheet dump

**Gate:** Eligibility comes from the backend only, PASS-only gating is obvious, and the frontend is ready for Phase 4 ranking screens without implementing ranking itself.
...
- Root layout: fonts, global CSS, nav shell, and shared chrome

#### `frontend/app/page.tsx`

- Landing/dashboard: strong hero, primary CTA, recent cases, and summary cards

**Gate check:** `npm run dev` starts, `/` renders without error, and the shell already feels demo-ready.
...
- `frontend/app/layout.tsx` — root shell and shared navigation
...
- `frontend/app/cases/page.tsx` — cases index
- `frontend/app/cases/[id]/layout.tsx` — case workspace shell
- `frontend/app/cases/[id]/page.tsx` — case overview and supplier summary
- `frontend/app/cases/[id]/requirements/page.tsx` — requirements UI
- `frontend/components/CaseTabs.tsx` — case workspace navigation
...
- `frontend/components/documents/DocumentSummaryPanel.tsx` — document status and trace summary
- `frontend/components/evidence/EvidenceDrawer.tsx` — evidence quote and source details
- `frontend/app/cases/[id]/page.tsx` — case overview surfaces for supplier documents and evidence
...
- `backend/app/services/eligibility/eligibility_service.py`
- `tests/unit/test_evaluator.py`
- `frontend/app/cases/[id]/eligibility/page.tsx` — eligibility matrix connected to real API
- `frontend/components/eligibility/EligibilityMatrix.tsx` — PASS/FAIL/REVIEW/PENDING table
- `frontend/components/evidence/EvidenceDrawer.tsx` — trace details from checks and documents

...
export function EvidenceDrawer({
  ...
  // Group evidence by field_key so conflicting claims for the same field
  // can be shown side-by-side instead of buried in a flat list.
  const groupedByField = evidenceList.reduce<Record<string, Evidence[]>>((acc, ev) => {
    const key = ev.field_key || ev.id;
    acc[key] = acc[key] || [];
    acc[key].push(ev);
    return acc;
  }, {});

  const conflictGroups = Object.entries(groupedByField).filter(
    ([, items]) => items.length > 1 && items.some((ev) => ev.state === "CONFLICTING")
  );
  const conflictedIds = new Set(conflictGroups.flatMap(([, items]) => items.map((ev) => ev.id)));
  const nonConflictEvidence = evidenceList.filter((ev) => !conflictedIds.has(ev.id));
      ...
      <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-start bg-gray-50/60 dark:bg-gray-800/60">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <StatusBadge status={checkStatus || 'PENDING'} />
          </div>
        ...
        <div>
          ...
          {evidenceList.length === 0 ? (
            ...
            <div className="space-y-6">
              {/* Conflicting Evidence: side-by-side VS comparison */}
              {conflictGroups.map(([fieldKey, items]) => (
                <div
                  key={fieldKey}
                  className="rounded-2xl border-2 border-red-300 dark:border-red-800 bg-red-50/40 dark:bg-red-950/20 p-5 space-y-4"
                >
                  <div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-wider text-red-700 dark:text-red-300">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                    </svg>
                    Conflicting Evidence — {fieldKey}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-stretch gap-4">
                    {items.slice(0, 2).map((ev, idx) => (
                      <div key={ev.id} className="contents">
                        <div className="p-4 rounded-xl bg-white dark:bg-gray-900 border border-red-200 dark:border-red-900/60 space-y-2">
                          <span className="text-[10px] font-mono uppercase font-bold text-gray-500 block">
                            Source {idx + 1}: Document {ev.document_id.slice(0, 8)}
                          </span>
                          <div className="text-lg font-black text-gray-900 dark:text-white">
                            {String(ev.normalized_value)} {ev.unit || ""}
                          </div>
                          {ev.quoted_text && (
                            <blockquote className="p-2.5 rounded-lg border-l-4 border-red-400 bg-red-50/60 dark:bg-red-950/30 text-xs italic text-gray-700 dark:text-gray-300">
                        ...
                        {idx === 0 && (
                          <div className="hidden md:flex items-center justify-center text-xs font-black text-red-500">
                            VS
                          </div>
                        )}
                      </div>
                  ...
                  <p className="text-xs text-red-800 dark:text-red-300 font-semibold pt-2 border-t border-red-200 dark:border-red-900/50">
                    Status: CONFLICTING EVIDENCE — human review required before this supplier can proceed to ranking.
                  </p>
                </div>
              ))}

              {nonConflictEvidence.map((ev) => (
                <div
                  key={ev.id}
                ...
                >
                  ...
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <StatusBadge status={ev.state === "SUPPORTED" ? "PASS" : ev.state === "CONFLICTING" ? "FAIL" : "REVIEW"} />

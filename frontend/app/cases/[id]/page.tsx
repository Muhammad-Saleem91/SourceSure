...
import { EligibilityMatrixResponse } from "@/lib/types";
export default function CaseOverviewPage({
  ...
  const [eligibility, setEligibility] = useState<EligibilityMatrixResponse | null>(null);
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const [supList, reqList, eligData] = await Promise.all([
        api.getSuppliers(id),
        api.getRequirements(id),
        api.getEligibilityMatrix(id).catch(() => null), // Catch error in case eligibility isn't run yet
      ]);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <SummaryCard
          ...
          subtext="Candidates in evaluation batch"
        />
        ...
        <SummaryCard
          ...
          subtext={
            <>
              {requirements.filter((r) => r.kind === "MANDATORY").length} Mandatory •{" "}
              {requirements.filter((r) => r.kind === "PREFERENCE").length} Preference
            </>
          }
        ...
        <SummaryCard
          title="Eligible for Ranking"
          value={passCount}
          subtext="Qualifies for ranking"
        ...
        <SummaryCard
          ...
          value={suppliers.filter((s) => s.status === "FAIL" || s.status === "REVIEW").length}
          subtext="Suppliers with blocking issues"
            ...
            {suppliers.map((s) => (
              ...
              >
                <div className="space-y-1">
                  ...
                  {/* Show failure/review reasons if eligibility data exists */}
                  {(s.status === "FAIL" || s.status === "REVIEW") && eligibility && (
                    <div className="mt-2 text-xs font-medium space-y-1">
                      {eligibility.suppliers
                        .find((sup) => sup.supplier_id === s.id)
                        ?.checks.filter((chk) => chk.status !== "PASS")
                        .map((check) => (
                          <div
                            key={check.id}
                            className={`flex items-start gap-1.5 ${
                              check.status === "FAIL" ? "text-red-600 dark:text-red-400" : "text-amber-600 dark:text-amber-400"
                            }`}
                          >
